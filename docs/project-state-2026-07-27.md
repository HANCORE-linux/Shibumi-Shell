...
> Machine2. `ARCHITECTURE.md` and `release-readiness.md` remain authoritative.
...
- Updated Machine2 transactionally. The final status panel moved from
  `opened:false/panelLoaded:false` to `true/true` and returned to `false/false`
  after hide. No test panel remained open.

## Stable Machine2 handoff

- current address: `drdeltree@192.168.2.202`
...
- All G1-G18 groups passed disable/enable recovery on Machine2.
...
- Final geometry contains 18 native slots, no duplicate IDs, Center 87 px,
  ...
  non-visible slot because Machine2 exposes no available GPU telemetry owner.

...
| G6 | Volume | Official `omarchy.audio` state/actions through a narrow bridge; one open-only local peak monitor | V1 full/compact pill and lazy V1 mixer panel cover output/input devices, per-app streams, mute, meter, and external mixer; real Wayland/top-bottom/multi-output acceptance remains |
| G7 | Claude, Codex, and OpenCode usage | One root-owned Shibumi AI usage service; official Claude/Codex scanners load only when their local data exists, and one read-only OpenCode SQLite provider fills the host gap | One selected-provider pill, V1 usage fill, local lazy panel, provider cycling, refresh, legacy `omarchy.model-usage` routing, and no duplicate cache/poller ownership; OpenCode is accepted on Machine2 while Claude/Codex real-account gates remain |
| G8 | Weather, clock, date, status indicators, Omarchy update | Shared Shibumi clock/status adapters plus official Quattro weather-detail, idle, notification, and update owners | Single V1 pill, lazy routed calendar, local weather/status/update facades, child settings, and monitor-local normal/compact/minimal stages are implemented. Top weather, update-available, idle/DND, the complete recording-indicator lifecycle, and a real Voxtype recording/transcription cycle pass on Machine2; bottom and physical multi-output acceptance remain. |
...
| G11 | Network | One root-owned service around the official `omarchy.network` backend; one panel-lifecycle saved-profile adapter fills the host API gap | V1 `NET`/SSID/signal and compact views plus a lazy local details/DNS/speed-test/available/saved/connect/forget panel; top Wayland mapping and cleanup pass on Machine2, while mutation, bottom, and physical multi-output acceptance remain |
| G12 | Battery | One root-owned Shibumi power service over Quickshell's UPower singleton; Quattro battery helper only while the detail panel is open | V1 compact/full battery view and lazy battery panel; hidden on batteryless desktops; a real discharging-to-charging transition passes on Machine2 |
| G13 | Brightness | One root-owned service around the complete official `omarchy.monitor` backend | V1 `BRI`/sun/percentage and compact views plus a lazy, screen-local Shibumi brightness/scale/display panel; top/Bottom mapping, reversible laptop brightness mutation, and a real `1.0 -> 1.25 -> 1.0` scale round trip pass on Machine2, while physical display enable/disable and multi-output acceptance remain |
...
| G15 | Bluetooth | One root-owned service around the complete official `omarchy.bluetooth` backend | V1 full/compact widget and lazy per-output Shibumi device panel; shared discovery lease and one symmetric legacy IPC target; top mapping and real adapter/radio/discovery lifecycle pass on Machine2, while real device/audio, bottom, and physical multi-output acceptance remain |
...
The unfinished V1 prototype at
`Quickshell-Dots/plugins/omarchy-menu/` is a product and interaction reference,
not a payload that can be copied into Shibumi. It currently has no manifest, owns
its own `PanelWindow`, depends on V1 root and theme properties, invokes commands
directly, references the V1 `load-apps.py` path, and persists V1 cache files.

The Shibumi implementation is the independent `hancore.shibumi.menu` plugin with
`menu` and `service` entry points:

- the host owns loading, summoning, focus, and plugin lifecycle;
- an app index service owns desktop-entry discovery and favorites;
- an action adapter owns validated Omarchy commands;
- menu views consume those contracts;
- G1 remains owned by `hancore.shibumi.control-center`, not by this plugin;
- all windows are host-loaded and screen-aware; no second `ShellRoot` exists.

The local implementation also owns the versioned favorites, hidden-app, icon,
scale, selection, and background preferences. Wallpaper state is read from the
existing first-party `omarchy.background` service; the menu does not create a
second watcher or background owner.

The missing helper and current Quattro command coverage were inventoried before
the port. V1-only Mako, Walker, Waybar, and SwayOSD actions are not carried
forward blindly.
...
5. Extract the App Menu as its own native plugin package.
...
exactly three retry starts before exposing failure. Machine2 accepts the
...
The unfinished V1 App Menu is in scope and is not blocked by the current bar
foundation. It is an independent plugin so future bar hosts can summon the
same menu without copying its windows or data services. G1 Control Center
ownership remains separate.

Notification and OSD replacement do not start with the App Menu port. Their
current Quattro services remain authoritative until an explicit, exclusive
owner-selection contract prevents duplicate notification servers or duplicate
OSD feedback.

...
| G11 | Network state, Wi-Fi/ethernet presentation, throughput, network panel | One process-wide native `hancore.shibumi.network` owner with primitive snapshots, exact profile actions, demand-driven scanner/telemetry/reachability, Shibumi QR, Enterprise, and route-bound speed-test seams | Ported in source, acceptance incomplete | The atomic source cutover and fixture gates cover lifecycle, identities, saved profiles, action completion/failure, QR decoding, Enterprise bounds, and speed-test races. Previous host-backed live evidence does not validate this exact native commit. Validate Wi-Fi/Ethernet mutations, Open/WPA2/WPA3, QR, reachability, speed test, NetworkManager loss/full process restart, bottom placement, and physical multi-output on Machine 2; Enterprise remains physically unconfirmed. |
...
| G15 | Bluetooth widget and panel | One shared Shibumi-native BlueZ/PipeWire adapter publishes detached device records and re-resolves device/adapter incarnations before one retained helper mutation path; local V1-style widgets and panels own no backend entities | Ported, acceptance incomplete | Fixture gates reject malformed, stale, replacement, and ambiguous identities with zero dispatch and require explicit typed-result `.ok` handling. Fresh pair/connect/disconnect/forget, Bluetooth audio, and physical multi-output acceptance remain required. |
...
2. Quattro owns plugin lifecycle, notifications, OSD, BlueZ, PipeWire,
   display control, and other platform services where an adequate service
   contract exists. Shibumi G11 is the declared process-wide NetworkManager
   capability owner over Quickshell's public API.

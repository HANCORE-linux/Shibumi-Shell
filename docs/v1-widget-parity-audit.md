...
| G11 | Network state, Wi-Fi/ethernet presentation, throughput, network panel | One root-owned official NetworkManager backend plus lazy Shibumi panels and a lifecycle-only saved-profile adapter | Ported, acceptance incomplete | Top and bottom panel placement, expanded metadata, saved-profile discovery, alias routing, backend refresh, real speed-test output, and cleanup pass on the validation system. A stale saved-only profile was forgotten through the visible two-step action and stayed absent after reopen/rescan. A real `DHCP -> Cloudflare -> DHCP` DNS round trip passed through the panel and Polkit with the original connection and resolver state restored. Validate a new protected-network connection/passphrase and physical multi-output before release. |
...
| G15 | Bluetooth widget and panel | One shared official backend with local V1-style widgets and lazy Shibumi device panels | Ported, acceptance incomplete | Top/Bottom mapping, real adapter/radio reactivity, discovery lifecycle, multi-panel lease accounting, and legacy IPC pass on the validation system. Validate pair/connect/disconnect/forget with real devices, Bluetooth audio, and physical multi-output. |

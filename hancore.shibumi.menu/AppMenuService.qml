...
Item {
  ...
  property var menuConfig: ({
    ...
    favorites: [],
    hidden: [],
    launcher: { mode: "text", text: "shibumi", icon: "omarchy" },
    presentation: {
      icons: true,
      scale: 100,
  ...
  readonly property bool ready: !!stateService && stateService.ready === true
  ...
  function configure(value) {
    if (!stateService || typeof stateService.normalizeMenuConfig !== "function")
      return false
    menuConfig = stateService.normalizeMenuConfig(value)
  ...
  function persistMenuConfig(value) {
    if (!stateService || typeof stateService.normalizeMenuConfig !== "function"
        || typeof stateService.setMenuConfig !== "function") return false
    const next = stateService.normalizeMenuConfig(value)
    const accepted = stateService.setMenuConfig(next)
    if (accepted) configure(next)
    return accepted
  }
  ...
  function setPresentation(name, value) {
    ...
    candidate[key] = value
    const normalized = stateService.normalizeMenuConfig({
      version: menuConfig.version,
  ...
  function syncConfiguration() {
    if (!stateService || !stateService.ready) return false
    return configure(stateService.config ? stateService.config.menu : null)
  }
  ...
  Connections {
    ...
    ignoreUnknownSignals: true
    function onRevisionChanged() { root.syncConfiguration() }
  }

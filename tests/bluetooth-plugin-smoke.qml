...
ShellRoot {
  ...
  Component {
    ...
    Fixtures.BluetoothTestPanel {}
  }

  Bluetooth.Service {
    id: sharedBluetoothService
    bar: fakeBar
    panelComponent: bluetoothPanelComponent
  }
    ...
    onTriggered: {
      ...
      if (root.phase === 0) {
        ...
        if (root.clickTargets.length !== 2 || sharedBluetoothService.sessionCount !== 0)
          return root.fail("duplicate official target or closed lifecycle")
        ...
        if (backend.toggleCount !== 2 || !first.radioEnabled
            || sharedBluetoothService.discovering)
          return root.fail("closed radio enable left discovery running")
        ...
        if (backend.connectCount !== 1 || backend.disconnectCount !== 1
          ...
          return root.fail("official device action forwarding")
        ...
        Qt.callLater(function() {
          ...
          console.log("bluetooth plugin smoke passed")
          Qt.quit()
        })
        stop()
      }

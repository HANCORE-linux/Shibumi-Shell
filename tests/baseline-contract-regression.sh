...
rg -Fq 'local baseline_version=${SHIBUMI_OMARCHY_BASELINE_VERSION:-4.0.4}' \
  "$helper" \
  || fail 'central baseline loader does not default to Omarchy 4.0.4'
for current_manifest_name in \
  omarchy-installed-package-v4.0.4.json \
  omarchy-installed-source-parity-v4.0.4.json; do
...
rg -Fq 'SHIBUMI_OMARCHY_BASELINE_VERSION=4.0.4' \
  "$installed_package_job" \
  || fail 'installed-package job does not pin Omarchy 4.0.4'
...
rg -Fq 'SHIBUMI_OMARCHY_BASELINE_VERSION=4.0.4' \
  "$installed_source_job" \
  || fail 'installed-source-parity job does not pin Omarchy 4.0.4'

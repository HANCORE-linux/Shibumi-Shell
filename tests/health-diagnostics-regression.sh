...
health_page="$repo_root/hancore.shibumi.control-center/ControlMainPage.qml"
...
for contract in \
  ...
  'def is_omarchy_path' \
  'runtime-errors-omarchy' \
  'runtime-errors-sensitive' \
  'suppressed_sensitive' \
  'issue_eligible=owner == OWNER_SHIBUMI'; do
...
for contract in \
  ...
  'return "[sensitive diagnostic redacted]"'; do
...
for contract in \
  ...
  'String(check.owner || "") !== "shibumi"'; do
  rg -Fq "$contract" "$health_page" || {
    printf 'health diagnostics regression failed: missing UI attribution contract %s\n' \
      "$contract" >&2
    exit 1
  }
done
...
if rg -q 'gh (issue create|api)|api\.github\.com|requests\.post|curl .*api\.github' \
    "$health_runner" "$health_page"; then
  printf 'health diagnostics regression failed: Health must not perform GitHub writes\n' >&2
  exit 1
fi

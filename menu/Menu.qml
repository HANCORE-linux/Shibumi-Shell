...
Item {
  ...
  readonly property var shibumiConfig: barController && barController.shibumiConfig
    ? barController.shibumiConfig : ({})
  readonly property var barPresentation: shibumiConfig.presentation || ({})
  readonly property var workspaceConfig: shibumiConfig.workspace || ({})
  readonly property string pickerStyle: shibumiConfig.picker
    ? String(shibumiConfig.picker.style || "tanzaku") : "tanzaku"
  ...
  readonly property int reactorMode: {
    if (barController && barController.reactorMode !== undefined)
      return Number(barController.reactorMode) || 0
    const config = shell && shell.shellConfig ? shell.shellConfig : null
    const shibumi = config && config.bar ? config.bar.shibumi : null
    return shibumi && shibumi.reactor ? Number(shibumi.reactor.mode) || 0 : 0
  }
  ...
  function groupSetting(groupId, key, fallback) {
    return barController && typeof barController.groupSetting === "function"
      ? barController.groupSetting(groupId, key, fallback) : fallback
  }

  function setGroupSetting(groupId, key, value) {
    return barController && typeof barController.setGroupSetting === "function"
      ? barController.setGroupSetting(groupId, key, value) : false
  }

  function setBarPresentation(name, value) {
    return barController
      && typeof barController.setPresentationSetting === "function"
      ? barController.setPresentationSetting(name, value) : false
  }

  function setWorkspacePreference(name, value) {
    return barController
      && typeof barController.setWorkspacePreference === "function"
      ? barController.setWorkspacePreference(name, value) : false
  }

  function setPickerStyle(value) {
    return barController && typeof barController.setPickerStyle === "function"
      ? barController.setPickerStyle(value) : false
  }

  function setBarPosition(value) {
    return barController && typeof barController.setBarPosition === "function"
      ? barController.setBarPosition(value) : false
  }

  function setAllSplits(value) {
    return barController && typeof barController.setAllSplits === "function"
      ? barController.setAllSplits(value) : false
  }

  function resetBarLayout() {
    return barController && typeof barController.resetBarLayout === "function"
      ? barController.resetBarLayout() : false
  }

  function accentColor(value) {
    const palette = barController ? barController.themePalette : null
    const id = String(value || "")
    if (id === "accent") return Commons.Color.accent
    if (id === "color02" && palette) return palette.color02
    if (id === "color03" && palette) return palette.color03
